...
function App() {
  ...
  useEffect(() => {
    ...
    const products = (byColumn.products || []).slice().sort(sortByOrder).map((option) => ({ optionKey: option.optionKey, abbreviation: option.abbreviation || option.optionKey, fullName: option.name, color: option.color }));
  ...
  const openProductManager = () => {
    setAdminMenuColumn(null);
    setProductDrafts(Object.fromEntries(productOptions.map((option) => [option.abbreviation, { abbreviation: option.abbreviation, fullName: option.fullName, color: option.color }])));
  ...
  const deleteProductOption = async (productKey) => {
    ...
    removeLabelOption('products', productKey);
      setProductOptions((current) => current.filter((option) => (option.optionKey || option.abbreviation) !== productKey));
    setProductDrafts((current) => {
      const nextDrafts = { ...current };
      delete nextDrafts[productKey];
      return nextDrafts;
  ...
  if (!isAuthLoaded || (isSignedIn && (!currentUser || workspaceRecords === undefined))) {
    return (
