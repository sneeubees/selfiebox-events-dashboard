...
function DashboardApp() {
  ...
  const creatorProfiles = useQuery(api.users.listCreatorProfiles, canAccessDashboard ? {} : 'skip');
  ...
  const commissionMonthOverrideRecords = useQuery(
    api.commissions.listMonthOverrides,
    canAccessDashboard && commissionDialog.isOpen && commissionDialog.month
      ? {
          month: commissionDialog.month,
          year: selectedWorkspaceYear,
        }
  ...
  const previousCommissionYearEvents = useQuery(
    api.events.listByWorkspaceYear,
    canAccessDashboard && commissionDialog.isOpen && commissionDialog.month === 'January' && selectedWorkspaceYear > 0
      ? { workspaceYear: selectedWorkspaceYear - 1 }
      : 'skip'
  );
  ...
  const creatorProfileMap = useMemo(
    () =>
      new Map(
  ...
  const attendantRecordMap = useMemo(() => Object.fromEntries(attendantOptions.map((option) => [option.fullName, option])), [attendantOptions]);
  ...
  const currentCommissionAttendantRecord = useMemo(
    () => attendantRecordMap[commissionDialog.attendant] || null,
    [attendantRecordMap, commissionDialog.attendant]
  );
  ...
  const commissionMonthOverrideMap = useMemo(() => {
    if (!Array.isArray(commissionMonthOverrideRecords)) {
      return {};
    }
    return commissionMonthOverrideRecords.reduce((accumulator, record) => {
      if (!accumulator[record.attendant]) {
        accumulator[record.attendant] = {};
      }
      accumulator[record.attendant][record.eventId] = {
  ...
  }, [commissionMonthOverrideRecords]);
  ...
  const commissionSummaryAttendantNames = useMemo(
    () => commissionAttendantNames.filter((name) => !attendantRecordMap[name]?.isFullTimeEmployee),
    [attendantRecordMap, commissionAttendantNames]
  );
  const commissionSheetEvents = useMemo(() => {
    if (!commissionDialog.attendant) {
      return [];
    }

    const isFullTimeEmployee = Boolean(currentCommissionAttendantRecord?.isFullTimeEmployee);
    if (!isFullTimeEmployee) {
      return commissionMonthEvents.filter((event) => (event.attendants || []).includes(commissionDialog.attendant));
    }

    const previousMonthName = getPreviousMonthName(commissionDialog.month);
    const currentYearEvents = events
      .filter((event) => (event.date ? new Date(event.date).getFullYear() === selectedWorkspaceYear : event.workspaceYear === selectedWorkspaceYear))
      .filter((event) => Array.isArray(event.attendants) && event.attendants.includes(commissionDialog.attendant))
      .filter((event) => {
        const eventMonth = getEventMonth(event);
        return eventMonth === commissionDialog.month || eventMonth === previousMonthName;
      });
    const previousYearJanuaryEvents = commissionDialog.month === 'January' && Array.isArray(previousCommissionYearEvents)
      ? previousCommissionYearEvents
          .filter((event) => Array.isArray(event.attendants) && event.attendants.includes(commissionDialog.attendant))
          .filter((event) => getEventMonth(event) === 'December')
      : [];

    return [...currentYearEvents, ...previousYearJanuaryEvents].sort((left, right) => sortEvents(left, right));
  }, [commissionDialog.attendant, commissionDialog.month, commissionMonthEvents, currentCommissionAttendantRecord?.isFullTimeEmployee, events, previousCommissionYearEvents, selectedWorkspaceYear]);
  const getCommissionOverrideForAttendant = useCallback((attendantName, eventId) => {
    if (commissionDialog.attendant === attendantName && commissionDialog.overrides[eventId]) {
      return commissionDialog.overrides[eventId];
    }
    return commissionMonthOverrideMap[attendantName]?.[eventId] || {};
  }, [commissionDialog.attendant, commissionDialog.overrides, commissionMonthOverrideMap]);
  const commissionRows = useMemo(() => {
    if (!commissionDialog.attendant) {
      return [];
    }
    const isFullTimeEmployee = Boolean(currentCommissionAttendantRecord?.isFullTimeEmployee);
    return commissionSheetEvents
      .filter((event) => isEventInCommissionPeriod(event, commissionDialog.month, selectedWorkspaceYear, commissionDialog.period, isFullTimeEmployee))
      .map((event) => {
        const automaticHoursPayable = getAutomaticCommissionHours(event, isFullTimeEmployee);
  ...
  }, [branchAddressMap, commissionDialog.attendant, commissionDialog.month, commissionDialog.period, commissionRates, commissionSheetEvents, currentCommissionAttendantRecord?.isFullTimeEmployee, getCommissionOverrideForAttendant, selectedWorkspaceYear]);
  ...
  const commissionSummaryRows = useMemo(() => {
    return commissionSummaryAttendantNames.map((attendantName) => {
      const rows = commissionMonthEvents
        .filter((event) => (event.attendants || []).includes(attendantName))
        .filter((event) => isEventInCommissionPeriod(event, commissionDialog.month, selectedWorkspaceYear, commissionDialog.period, false))
        .map((event) => {
          const override = getCommissionOverrideForAttendant(attendantName, event.id);
          const hoursPayable = override.hoursPayable === '' || override.hoursPayable === undefined
            ? getAutomaticCommissionHours(event, false)
  ...
  }, [commissionDialog.month, commissionDialog.period, commissionMonthEvents, commissionRates, commissionSummaryAttendantNames, getCommissionOverrideForAttendant, selectedWorkspaceYear]);
  ...
  useEffect(() => {
    if (!commissionDialog.isOpen || !currentCommissionAttendantRecord?.isFullTimeEmployee || commissionDialog.period === 'all') {
      return;
    }
    setCommissionDialog((current) => ({ ...current, period: 'all' }));
  }, [commissionDialog.isOpen, commissionDialog.period, currentCommissionAttendantRecord?.isFullTimeEmployee]);
  ...
  const addBlankEvent = (monthName) => {
    if (currentUser?.role !== 'admin' && isPastMonth(monthName, selectedWorkspaceYear)) {
      openNotice('Only admins can add events in the past.');
      return;
    }
    const newEvent = {
      ...eventDefaults,
  ...
  const exportCommissionSheet = async () => {
    ...
    try {
      const { blob, fileName } = await exportCommissionPdf({
        ...
        period: commissionDialog.period,
        attendant: commissionDialog.attendant,
        isFullTimeEmployee: Boolean(currentCommissionAttendantRecord?.isFullTimeEmployee),
        rows: commissionRows,
  ...
  const toggleProductOnEvent = (eventId, productKey) => {
    updateEvent(eventId, (event) => {
      ...
      if (hasProduct) {
        const nextQuantities = { ...(event.productQuantities || {}) };
      ...
      return {
        ...event,
        products: [...event.products, productKey],
      };
    });
  };

  const setProductQuantity = (eventId, productKey, nextQty) => {
    const clamped = Math.max(1, Math.min(9, Math.round(nextQty) || 1));
    updateEvent(eventId, (event) => ({
      ...event,
      productQuantities: { ...(event.productQuantities || {}), [productKey]: clamped },
    }));
  };
  ...
  const saveAttendantOption = (attendantKey) => {
    ...
    void upsertLabelOptionMutation({
      ...
      isFullTimeEmployee: Boolean(draft?.isFullTimeEmployee),
      color: branchStyle?.background || '#dfe7f6',
  ...
  return (
      <div className="app-shell">
        ...
        <header className="topbar compact">
          <div className="topbar-brand">
            <span className="topbar-logo" role="img" aria-label="SelfieBox"><span className="topbar-logo-selfie">Selfie</span><span className="topbar-logo-box">Box</span><span className="topbar-logo-reg" aria-hidden="true">®</span><span className="topbar-logo-spark" aria-hidden="true"><img src="/images/logo/selfiebox-original-x-stripes-solid.webp" alt="" width="180" height="176" /></span></span>
      ...
      {filtersOpen ? <ModalShell title="Filters" onClose={() => setFiltersOpen(false)} hideCloseButton><div className="filter-popup-scroll"><div className="filter-popup"><FilterGroup title="Branches" options={branchOptions.map((option) => ({ value: option.abbreviation, label: option.fullName }))} selected={selectedBranches} onToggle={(value) => toggleSelection(setSelectedBranches, value)} styles={branchStyles} /><FilterGroup title="Products" options={productOptions.map((option) => ({ value: option.abbreviation, label: option.fullName }))} selected={selectedProducts} onToggle={(value) => toggleSelection(setSelectedProducts, value)} styles={productStyles} /><FilterGroup title="Statuses" options={statusNames} selected={selectedStatuses} onToggle={(value) => toggleSelection(setSelectedStatuses, value)} styles={statusStyles} /><FilterGroup title="Payment" options={getManagedOptionNames(managedSingleOptions, 'paymentStatus')} selected={selectedPayments} onToggle={(value) => toggleSelection(setSelectedPayments, value)} styles={managedSingleStyles.paymentStatus || {}} /><FilterGroup title="Attendants" options={branchScopedAttendantOptions.map((option) => ({ value: option.fullName, label: option.displayName || option.fullName }))} selected={selectedAttendants} onToggle={(value) => toggleSelection(setSelectedAttendants, value)} styles={attendantStyles} /></div></div><div className="modal-actions filter-popup-actions"><button className="ghost-button" type="button" onClick={clearFilters}>Clear filter</button><button className="ghost-button filter-save-button" type="button" onClick={openSaveCustomViewModal}>Save Custom View</button><button className="primary-button" type="button" onClick={() => setFiltersOpen(false)}>Apply</button></div></ModalShell> : null}
          ...
          <div className="commission-sheet">
            <div className="commission-toolbar">
              ...
              <div className="commission-periods">
                {currentCommissionAttendantRecord?.isFullTimeEmployee ? (
                  <span className="commission-period-badge">26th previous month to 25th current month</span>
                ) : (
                  <>
                    <button
            ...
            <div className="commission-subhead">
              ...
              <span>{getCommissionPeriodLabel(commissionDialog.month, selectedWorkspaceYear, commissionDialog.period, Boolean(currentCommissionAttendantRecord?.isFullTimeEmployee))}</span>
      ...
      {productEditorEventId && selectedProductEvent ? <ModalShell title="Select product" onClose={() => setProductEditorEventId(null)}><div className="branch-manager"><div className="branch-selector-list product-selector-list">{productOptions.map((option) => { const isSelected = selectedProductEvent.products.includes(option.abbreviation); const qty = selectedProductEvent.productQuantities?.[option.abbreviation] || 1; return <div className="product-selector-item" key={option.optionKey || option.abbreviation}><button className={["branch-selector-item", isSelected ? "is-selected" : ""].join(" ").trim()} type="button" title={option.fullName} onClick={() => toggleProductOnEvent(selectedProductEvent.id, option.abbreviation)}><span className="branch-color-chip" style={{ background: option.color, color: getContrastColor(option.color) }}>{option.fullName}</span></button>{isSelected ? <div className="qty-stepper"><button type="button" className="qty-stepper-btn" disabled={qty <= 1} onClick={(event) => { event.stopPropagation(); setProductQuantity(selectedProductEvent.id, option.abbreviation, qty - 1); }} aria-label={`Decrease ${option.fullName} quantity`}>-</button><span className="qty-stepper-value">{qty}</span><button type="button" className="qty-stepper-btn" disabled={qty >= 9} onClick={(event) => { event.stopPropagation(); setProductQuantity(selectedProductEvent.id, option.abbreviation, qty + 1); }} aria-label={`Increase ${option.fullName} quantity`}>+</button></div> : null}</div>; })}</div><div className="modal-actions"><button className="primary-button" type="button" onClick={() => setProductEditorEventId(null)}>Done</button></div></div></ModalShell> : null}
...
function renderCell({ columnKey, event, openDrawer, updateEventField, updateEventLocationText, applyEventLocation, updateEventCustomField, dateEditor, setDateEditor, openDateEditor, closeDateEditor, applyEventDate, openBranchSelector, openProductSelector, openStatusSelector, openManagedSingleSelector, openAttendantSelector, openCustomOptionSelector, branchStyles, branchFullNames, productStyles, productFullNames, statusStyles, managedSingleStyles, attendantStyles, customItemStyles, customColumns, customColumnWidths, setActiveRowId, openLocationPreview, mainNameSuggestions, hoursSuggestions, canEdit, allowPastDates, creatorProfileMap }) {
    if (columnKey === 'name') {
      ...
      const creatorProfilePic = creatorProfile?.profilePic || '';
      const isNewWebRequest = event.status === 'Web Request';
      const hasStatusChanger = Boolean(event.firstStatusChangeByUserId);
      const statusChangerProfile = hasStatusChanger ? creatorProfileMap.get(String(event.firstStatusChangeByUserId || '')) || null : null;
      const statusChangerName = statusChangerProfile?.fullName || event.firstStatusChangeByName || '';
      const statusChangerPic = statusChangerProfile?.profilePic || event.firstStatusChangeByProfilePic || '';
...
function DateInlineEditor({ value, allowPastDates = false, onChange, onCancel, onApply }) {
  ...
  return createPortal(<div className="date-inline-modal" onMouseDown={onCancel} onClick={onCancel}><div className="date-inline-popover calendar-popover" onMouseDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
    ...
    <div className="calendar-grid">{cells.map((cell, index) => cell ? <button key={cell.dateValue} className={['calendar-day', value === cell.dateValue ? 'is-selected' : '', !allowPastDates && cell.dateValue < todayValue ? 'is-disabled' : ''].join(' ').trim()} type="button" disabled={!allowPastDates && cell.dateValue < todayValue} onClick={() => onChange(cell.dateValue)}>{cell.day}</button> : <span key={`blank-${index}`} className="calendar-day is-empty" />)}</div>
...
function CompactTagList({ items, styles, wide = false, quantities }) {
  if (!items || items.length === 0) return <span className="empty-cell-value" />;
  const visibleItems = items.slice(0, 2);
  const overflowCount = items.length - visibleItems.length;
  return <div className={`compact-tag-wrap${wide ? ' is-wide' : ''}`}>{visibleItems.map((item, index) => { const qty = quantities?.[item]; const label = qty > 1 ? `${item} ×${qty}` : item; return <div className="compact-tag-slot" key={String(item) + '-' + index}><Tag value={item} label={label} styles={styles} />{index === 1 && overflowCount > 0 ? <span className="extra-pill extra-pill-corner">+{overflowCount}</span> : null}</div>; })}</div>;
}

function FilterGroup({ title, options, selected, onToggle, styles = {} }) {
  const normalizedOptions = options.map((option) => typeof option === 'string' ? { value: option, label: option } : option);
  const shouldScroll = normalizedOptions.length > 10;
  return <section className="filter-group"><h4>{title}</h4><div className={["filter-options", shouldScroll ? "is-scrollable" : ""].join(" ").trim()}>{normalizedOptions.map((option) => {
    const isSel = selected.includes(option.value);
    const style = isSel ? styles[option.value] : null;
    return <label key={option.value} className={`filter-option${isSel ? ' is-selected' : ''}`} style={style && style.background ? { background: style.background, color: style.color } : undefined}><input type="checkbox" checked={isSel} onChange={() => onToggle(option.value)} /><span title={option.label}>{option.label}</span></label>;
  })}</div></section>;
}
...
function BookingDrawerSummary({ booking }) {
  ...
  const changeLog = Array.isArray(booking?.changeLog) ? booking.changeLog : [];
  const rows = [
  ...
  return (
    <div className="booking-summary-grid">
      ...
      {changeLog.length ? <div className="booking-summary-row booking-summary-submissions"><span>Change Log</span><div className="booking-changelog-list">{changeLog.map((entry) => (
        <div className="booking-changelog-entry" key={entry.id}>
          <div className="booking-changelog-meta">
            {entry.createdAtLabel}
            {' · '}
            {entry.source === 'dashboard' ? `Dashboard (${entry.actorName || 'Unknown user'})` : `External${entry.sourceIp ? ` (${entry.sourceIp})` : ''}`}
          </div>
          <ul className="booking-changelog-fields">
            {entry.changes.map((change) => (
              <li key={change.field}><strong>{change.label}:</strong> {change.from ? change.from : <em>empty</em>} {'→'} {change.to ? change.to : <em>empty</em>}</li>
            ))}
          </ul>
        </div>
      ))}</div></div> : null}
...
// Search Console has its own ranges (its data lags ~2 days, so windows end 2 days
// back and it has no meaningful "today"). Each maps to absolute GSC dates.
const SEO_PERIODS = [
  ['7d', '7 days'],
  ['28d', '28 days'],
  ['3mo', '3 months'],
  ['launch', 'Since new site'],
];
function seoRange(key) {
  const end = daysAgoISO(2);
  if (key === 'launch') return { startDate: SITE_LAUNCH_DATE, endDate: end, label: 'since new site' };
  const days = key === '7d' ? 7 : key === '3mo' ? 90 : 28;
  const label = key === '7d' ? 'last 7 days' : key === '3mo' ? 'last 3 months' : 'last 28 days';
...
function TurnoverView({ isAdmin, turnover }) {
  ...
  const { region, setRegion, regionOptions, netProfitPct, setNetProfitPct, rows, exportToExcel, isLoading } = turnover;
  const regionLabel = (regionOptions.find((o) => o.value === region) || {}).label || region;
  // While this year's figures are still coming from the server, the totals and
  // difference rows would be computed from history only - hide them and say so,
  // rather than showing a wrong number that later "jumps".
  const visibleRows = isLoading ? rows.filter((row) => !['totals', 'diff', 'diffPct'].includes(row.rowType || '')) : rows;
  return (
    <div className="statspage-view statspage-view-wide">
      <header className="statspage-viewhead"><div><h2>Turnover Figures</h2></div></header>
      {isLoading ? <div className="inline-loading turnover-loading" role="status"><span className="inline-spinner" aria-hidden="true" />Loading this year&rsquo;s figures and the totals&hellip; (older years are shown already)</div> : null}
...
function WebsiteStatsView({ isAdmin, connectUrl, openConnect }) {
  ...
  useEffect(() => { void load('last7', false, false); /* eslint-disable-next-line */ }, []);
  ...
  const head = (
    <header className="statspage-viewhead">
      <div><h2>Website Stats</h2><p>Live visitor activity from Google Analytics.</p></div>
      <div className="webstats-controls">
        <div className="webstats-tabs">
  ...
  let inner;
  if (loading && !data) {
    inner = <div className="webstats-empty">Loading your website stats…</div>;
...
function SeoStatsView({ isAdmin, connectUrl, openConnect }) {
  ...
  const load = useCallback(async (p, force) => {
    ...
    const r = seoRange(p);
    try {
      const res = await runSeo({ startDate: r.startDate, endDate: r.endDate });
  ...
  useEffect(() => { void load('28d', false); /* eslint-disable-next-line */ }, []);

  const seo = byPeriod[period];
  const periodLabel = seoRange(period).label;
  const tabs = (
    <div className="webstats-tabs">
      {SEO_PERIODS.map(([k, l]) => <button key={k} type="button" className={period === k ? 'is-active' : ''} onClick={() => void load(k, false)}>{l}</button>)}
    </div>
  );
  const head = (
    <header className="statspage-viewhead">
      <div><h2>SEO Stats</h2><p>Where you rank on Google and how it&apos;s moving. Search Console data is ~2 days delayed.</p></div>
      {tabs}
    </header>
  );
  ...
  let inner;
  if (loading && !seo) {
    inner = <div className="webstats-empty">Loading search rankings…</div>;
  } else if (seo && seo.needsSeoScope) {
    inner = <div className="webstats-empty"><p>SEO rankings need one extra Google permission (Search Console).</p>
      ...
      <button className="ghost-button" type="button" onClick={() => void load(period, true)}>Refresh</button></div>;
  } else if (seo && seo.connected === false) {
    inner = <div className="webstats-empty"><p>Connect Google to see search rankings here.</p>
      {isAdmin ? <button className="primary-button" type="button" disabled={!connectUrl} onClick={openConnect}>Connect Google</button> : <p className="webstats-muted">Ask an admin to connect Google.</p>}</div>;
  } else if (seo && seo.noProperty) {
    inner = <div className="webstats-empty"><p className="webstats-muted">No Search Console property found for this Google account. Add &amp; verify selfiebox.co.za in Search Console, then Refresh.</p>
      <button className="ghost-button" type="button" onClick={() => void load(period, true)}>Refresh</button></div>;
  } else if (seo && (seo.error || !seo.totals)) {
    inner = <div className="webstats-empty"><p>Couldn&apos;t load SEO data{seo.detail ? ` (${seo.detail})` : ''}.</p><button className="ghost-button" type="button" onClick={() => void load(period, true)}>Retry</button></div>;
  } else if (seo && seo.totals) {
    const t = seo.totals;
    const maxQ = seo.queries?.length ? Math.max(...seo.queries.map((q) => q.impressions), 1) : 1;
    const maxP = seo.pages?.length ? Math.max(...seo.pages.map((p) => p.clicks), 1) : 1;
    inner = <div className={`webseo${loading ? ' is-refetching' : ''}`}>
      ...
      <div className="webstats-section">
        <h4>Search terms <span>{periodLabel}</span></h4>
        {seo.queries?.length ? <div className="webseo-queries">
          ...
          {seo.queries.map((q) => <div className="webseo-q" key={q.query}>
            <span className="webseo-q-term" title={q.query}>{q.query}<em className="webseo-q-clicks">{q.clicks} clicks · {q.impressions.toLocaleString()} shown</em>
              <span className="webseo-q-track"><span style={{ width: `${(q.impressions / maxQ) * 100}%` }} /></span></span>
      ...
      {seo.pages?.length ? <div className="webstats-section">
        <h4>Top landing pages <span>{periodLabel}</span></h4>
        {seo.pages.map((p) => <div className="webstats-row" key={p.page}><div className="webstats-row-head"><span className="webstats-row-label" title={p.page}>{prettyDomain(p.page) || p.page}</span><span className="webstats-row-val">{p.clicks.toLocaleString()} clicks · pos {fmtPos(p.position)}</span></div><div className="webstats-track"><span style={{ width: `${(p.clicks / maxP) * 100}%` }} /></div></div>)}
      </div> : null}
      <div className="webstats-foot">
        <span>Google Search Console{seo.property ? ` · ${prettyDomain(seo.property)}` : ''}{seo.range ? ` · ${fmtGaDate(seo.range.start.replace(/-/g, ''))}–${fmtGaDate(seo.range.end.replace(/-/g, ''))}` : ''}</span>
...
function ComingSoonView({ title, blurb }) {
  return <div className="statspage-view">
    <header className="statspage-viewhead"><div><h2>{title}</h2></div></header>
    <div className="statspage-soon-panel">
      <div className="statspage-soon-badge">Coming soon</div>
      <p>{blurb}</p>
    </div>
  </div>;
}
...
function Tag({ value, label, styles, width, placeholder = 'Select', className = '' }) {
  ...
  const resolved = styles[value] || { background: '#d6d6d6', color: '#223042' };
  return <span className={["tag", className].join(" ").trim()} style={baseStyle ? { ...resolved, ...baseStyle } : resolved}>{label ?? value}</span>;
}
...
function isPastMonth(monthName, workspaceYear) {
  const monthIndex = monthNames.indexOf(monthName);
  if (monthIndex === -1) {
    return false;
  }

  const monthEnd = new Date(workspaceYear, monthIndex + 1, 0);
...
function parseCommissionTimeRange(value) {
  const text = String(value || '').trim();
  if (!text) {
    return null;
  }
...
function parseCommissionDate(dateString) {
...
function getFullTimeCommissionRange(month, year) {
  const monthIndex = monthNames.indexOf(month);
  if (monthIndex < 0) {
    return null;
  }
  return {
    start: new Date(year, monthIndex - 1, 26),
    end: new Date(year, monthIndex, 25, 23, 59, 59, 999),
  };
}

function isEventInCommissionPeriod(event, month, year, period, isFullTimeEmployee = false) {
  ...
  if (isFullTimeEmployee) {
    const range = getFullTimeCommissionRange(month, year);
    if (!range) {
      return false;
    }
  ...
  const monthIndex = monthNames.indexOf(month);
  if (monthIndex < 0 || eventDate.getFullYear() !== year || eventDate.getMonth() !== monthIndex) {
    return false;
  }
...
function getAutomaticCommissionHours(event, isFullTimeEmployee = false) {
  ...
  const overlapStart = Math.max(timeRange.startMinutes, workdayStart);
  const overlapEnd = Math.min(timeRange.endMinutes, workdayEnd);
...
function getCommissionPeriodLabel(month, year, period, isFullTimeEmployee = false) {
  if (isFullTimeEmployee) {
    const range = getFullTimeCommissionRange(month, year);
    if (!range) {
      return `${month} ${year}`;
    }
    return `${range.start.getDate()} ${monthNames[range.start.getMonth()]} ${range.start.getFullYear()} - ${range.end.getDate()} ${monthNames[range.end.getMonth()]} ${range.end.getFullYear()}`;
  }
...
function buildCommissionPdfFilename(month, year, period, attendant, isFullTimeEmployee = false) {
  const periodLabel = sanitizeFilenamePart(
    isFullTimeEmployee
      ? `26th prev month to 25th ${month} ${year}`
      : period === 'firstHalf'
        ? `15th ${month} ${year}`
        : period === 'secondHalf'
...
async function exportCommissionPdf({ month, year, period, attendant, isFullTimeEmployee = false, rows, totals }) {
  ...
  const periodLabel = getCommissionPeriodLabel(month, year, period, isFullTimeEmployee);
  ...
  const fileName = buildCommissionPdfFilename(month, year, period, attendant, isFullTimeEmployee);
