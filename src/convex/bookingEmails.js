...
export const sendBookingSubmissionEmail = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const sentFooter = `Sent from ${payload.sourceIp || "-"} on ${new Date(payload.submittedAt || Date.now()).toLocaleString("en-ZA", { timeZone: "Africa/Johannesburg" })} from SelfieBox events dashboard.`;
    const html = `
